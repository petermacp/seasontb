---
title: "Seasonality of tuberculosis in Blantyre, Malawi"
...
d %>%
  mutate(year = year(y_w)) %>%
  mutate(month = month(y_w, label=TRUE)) %>%
  group_by(year) %>%
  slice_max(i, n=5) %>%
  select(year, month, i) %>%
  group_by(month) %>%
...
plot(pred.rain, "slices", type="p", pch=19, cex=1.3, var=18, col="purple", ci.level=0.95, ci="bars", ylab="Report ratio", xlab="Weekly lag", main="B")
plot(pred.rain, "slices", type="p", pch=19, cex=1.3, var=30, col="purple", ci.level=0.95, ci="bars", ylab="Report ratio", xlab="Weekly lag", main="C")
...
plot(pred.temp, "slices", type="p", pch=19, cex=1.3, var=20, col="red2", ci.level=0.95, ci="bars", ylab="Report ratio", xlab="Weekly lag", main="E")
plot(pred.temp, "slices", type="p", pch=19, cex=1.3, var=30, col="red2", ci.level=0.95, ci="bars", ylab="Report ratio", xlab="Weekly lag", main="F")
...
#predict (nowcast) the effect on TB notification of rainfall/temperature
pred.rain <- crosspred(cb.rain, model.r, cen=0, by=0.2, ci.level = 0.95)
pred.temp <- crosspred(cb.temp, model.t, cen=17, by=0.2)
...
fig2 %>%
  ...
  labs(y="TB notification ratio (95% CI)",
       x="Lag (weeks)") +
...
fig2 %>%
  ...
  theme(legend.position = "none",
        ...
        panel.grid.minor = element_blank(),
        strip.background = element_rect(colour = "black", fill = "white"),
        strip.text = element_text(size=8, family = "Arial", color = "black"))
